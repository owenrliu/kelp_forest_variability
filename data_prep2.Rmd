...
# Finally, replace negative values with NA, indicating "not protected"
dat.all3$yrs.prot[dat.all3$yrs.prot<0] <- NA
...
dat.all4 <- dat.all3 %>% mutate(otteryear=NA) %>%
  ...
  # Assign 1985 to PISCO mainland sites
  mutate(otteryear=ifelse(dataset=="PISCO"& island=="Mainland",1985,otteryear)) %>%
